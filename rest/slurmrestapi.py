...
def get_cluster():
    ...
    cluster['nodes'] = len(list(nodes.keys()))
    cluster['cores'] = 0
...
def get_jobs_by_node_id(node_id):
    ...
    # filter jobs by node
    for jobid, job in jobs.items():
        nodes_list = list(job['cpus_allocated'].keys())
...
def sinfo():
    ...
    for name, attr in partitions.items():
        ...
        for node in list(NodeSet(attr['nodes'])):
            ...
            if key not in list(sinfo_data.keys()):
                sinfo_data[key] = []
...
@app.route('/proxy')
def proxy():
    masters = ", ".join(["\"%s\": \"*\"" % s for s in origins.split(',')])
