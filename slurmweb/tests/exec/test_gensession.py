...
from slurmweb.exec.gensession import SlurmwebExecGenSessionKey
from slurmweb.apps import SlurmwebAppSeed
from slurmweb.apps._defaults import SlurmwebAppDefaults


class TestGenSessionKeyExec(unittest.TestCase):
    def _parse(self, args):
        parser = SlurmwebExecMain.register_args()
        return parser.parse_args(
            ["gen-session-key", *args],
            namespace=SlurmwebAppSeed(),
        )
    ...
    @mock.patch("slurmweb.apps.gensession.SlurmwebAppGenSessionKey")
    def test_app(self, mock_slurmweb_app):
        seed = self._parse([])
        app = SlurmwebExecGenSessionKey.app(seed)
