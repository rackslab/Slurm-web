...
Minimal LDAP example in [.path]#`/etc/slurm-web/gateway.ini`#:
...
Adapt `SERVER` and search bases to your directory. Restart the gateway after
changes:
...
The login page is then presented on Slurm-web access:

image:slurm-web_login.png[]

TIP: LDAP issues? Use xref:usage:manpages/slurm-web-ldap-check.adoc[`slurm-web
ldap-check`] and xref:conf:auth/ldap.adoc#troubleshooting[LDAP troubleshooting].

.More details
****
* xref:conf:auth/index.adoc[Authentication documentation]
* xref:conf:conf/gateway.adoc[Gateway configuration reference documentation]
****
...
[user]
actions=stats-view,jobs-view,jobs-view-past
...
* *admin* for _njones_ user. This role has permission on `partitions-view`,
  `qos-view`,`accounts-view` and `reservations-view` actions.
...
Another user in _biology_ group can only view jobs:
