...
[user]
actions=view-stats,view-jobs,jobs-view-past,view-accounts
...
* *user* role is assigned to all members of _rd_ and _it_ LDAP groups. Users
  with this role have permissions on `view-stats`, `view-jobs`, `jobs-view-past`
  and `view-accounts` actions.
...
* *user* role is assigned to all users authenticated on LDAP directory. This
  role gives permissions on `view-stats`, `view-jobs`, `jobs-view-past` and
