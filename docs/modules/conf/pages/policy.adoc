...
* *user* role is assigned to all members of _rd_ and _it_ LDAP groups. Users
  with this role have permissions on `stats-view`, `jobs-view`, `jobs-view-past`
  and `accounts-view` actions.
* *admin* role is assigned to all members of _it_ LDAP group. Users with this
  role haves permissions on `nodes-view`, `partitions-view`, `qos-view` and
  `reservations-view` actions.
